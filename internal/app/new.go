...
func New(params Params) (*App, error) {
	...
	critErrChan := make(chan error, len(params.Runnables))
	...
	// Validate and set the logger and critical error channel for each runnable
	for _, runnable := range params.Runnables {
		...
		// Set the critical error channel for each runnable
		setRunnableCritErrChan(runnable, critErrChan)
	}
